...
function Overview({ data, assignedCount, completionRate, averageScore, setTab }: { data: AdminData; assignedCount: number; completionRate: number; averageScore: number; setTab: (tab: AdminTab) => void }) {
  ...
  return <>
    ...
    <div className="admin-overview-grid">
      ...
      <section className="admin-panel course-health"><div className="admin-panel-title"><div><span>COURSE HEALTH</span><h2>Intro 101 pilot</h2></div><b className="published">PUBLISHED</b></div><div className="course-mini-scene" aria-hidden="true"><i /><i /><span>AI?</span></div><ul><li><span>1</span> playable episode</li><li><span>10m</span> pilot duration</li><li><span>{data.settings.passingScore || "80"}%</span> passing score</li></ul><button className="secondary-button" type="button" onClick={() => setTab("courses")}>Manage course</button></section>
    </div>
  </>;
}
