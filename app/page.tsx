...
function useCountUp(target: number, durationMs = 900) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      // Reduced-motion users should see the final value immediately.
...
function MissionRail({
  ...
  return (
    <aside className="mission-rail" aria-label={t.courseTitle}>
      ...
      <button className={`mission-link guide-link ${labActive ? "active" : ""}`} type="button" onClick={onLab}>
        <span>🤖</span>
        <p>{lang === "es" ? "Laboratorio de IA" : "AI Practice Lab"}<small>{lang === "es" ? "Coach en vivo seguro" : "Safe live coach"}</small></p>
      </button>
  ...
  useEffect(() => {
    ...
    if (savedProfile) {
      try {
        ...
        // Browser-owned profile state is intentionally synchronized after hydration.
        // eslint-disable-next-line react-hooks/set-state-in-effect
